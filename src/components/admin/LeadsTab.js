...
function LeadCard({ lead, open, onToggle }) {
  ...
  const replySubject = encodeURIComponent('Re: Your Austin real estate inquiry — Marcus Cole Group');
  const replyBody = encodeURIComponent(`Hi ${lead.name},\n\nThank you for reaching out about your Austin home ${lead.intent === 'sell' ? 'sale' : lead.intent === 'buy' ? 'search' : 'real estate needs'}!\n\n—\nMarcus Cole\nMarcus Cole Group\n(512) 800-0101`);
      ...
      {open && (
        <div className="leads-card__body">
          <dl className="leads-card__fields">
            ...
            <div><dt>Intent</dt><dd>{lead.intent ? lead.intent.charAt(0).toUpperCase() + lead.intent.slice(1) : '—'}</dd></div>
